...
// MOCK: hardcoded studentId = 1 — replace with useAuth() derived ID
...
export default function StudentCollegeDetailClient() {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {sorted.map((student, idx) => {
                const isOwnRow = student.id === MOCK_STUDENT_ID;
                return (
                  <tr
                    key={student.id}
                    className={`${idx % 2 === 1 ? "bg-slate-50" : "bg-white"} ${
                      isOwnRow ? "ring-1 ring-inset ring-indigo-200" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-3 font-medium text-slate-800 whitespace-nowrap">
                      {student.name}
                      {isOwnRow && (
                        <span className="ml-2 px-1.5 py-0.5 bg-indigo-100 text-indigo-600 text-[10px] font-bold rounded">
                          You
                        </span>
                      )}
                    </td>
