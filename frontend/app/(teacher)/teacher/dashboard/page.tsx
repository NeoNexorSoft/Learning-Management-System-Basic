...
import { BookOpen, Users, TrendingUp, Star, ChevronRight, CheckCircle2, Clock, Loader2 } from "lucide-react"
...
function StatCard({ icon: Icon, label, value, sub, iconBg, iconColor }: {
  icon: ElementType; label: string; value: string | number; sub: string; iconBg: string; iconColor: string
}) {
  return (
  ...
  return (
    <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
      <td className="py-3.5 px-4">
        <p className="font-medium text-slate-800 text-sm">{enrollment.student?.name ?? "Student"}</p>
      ...
      <td className="py-3.5 px-4 hidden md:table-cell">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-slate-600">{enrollment.progress ?? 0}%</span>
        </div>
      </td>
      <td className="py-3.5 px-4">
  ...
  return (
    <div className="flex gap-3 py-4 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-400 mb-1">{review.course?.title ?? ""}</p>
...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Fetch enrollments and reviews for first few courses
        if (courseList.length > 0) {
          const firstCourseIds = courseList.slice(0, 3).map((c: any) => c.id)
          const [enrollRes, ...reviewResults] = await Promise.all([
            api.get(`/api/courses/${firstCourseIds[0]}/enrolled-students?limit=5`),
  ...
  const avgRating    = ratedCourses.length > 0
    ? (ratedCourses.reduce((s: number, c: any) => s + (c.avgRating ?? 0), 0) / ratedCourses.length).toFixed(1)
    : "—"
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">
            You have{" "}
            <span className="font-semibold text-indigo-600">{totalStudents} total student{totalStudents !== 1 ? "s" : ""}</span>{" "}
            across {totalCourses} course{totalCourses !== 1 ? "s" : ""}.
          </p>
        </div>

        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          <StatCard icon={BookOpen}   label="Total Courses"  value={totalCourses}                         sub="All your courses"       iconBg="bg-indigo-50"  iconColor="text-indigo-600" />
          <StatCard icon={Users}      label="Total Students" value={totalStudents}                        sub="Across all courses"     iconBg="bg-emerald-50" iconColor="text-emerald-600" />
          <StatCard icon={TrendingUp} label="Total Earnings" value={`TK${totalEarnings.toLocaleString()}`} sub="BDT lifetime earnings"  iconBg="bg-amber-50"   iconColor="text-amber-600" />
          <StatCard icon={Star}       label="Avg Rating"     value={avgRating}                           sub="Across rated courses"   iconBg="bg-blue-50"    iconColor="text-blue-600" />
        </div>
        ...
        <div className="grid xl:grid-cols-3 gap-6 mb-6">
          <div className="xl:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/teacher/enrollments" className="flex items-center gap-1 text-sm text-indigo-600 font-semibold hover:text-indigo-700">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {enrollments.length === 0 ? (
                    <tr><td colSpan={4} className="py-8 text-center text-slate-400 text-sm">No enrollments yet.</td></tr>
                  ) : (
                    enrollments.slice(0, 5).map((e: any) => <EnrollmentRow key={e.id} enrollment={e} />)
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/teacher/reviews" className="flex items-center gap-1 text-sm text-indigo-600 font-semibold hover:text-indigo-700">
                View all <ChevronRight className="w-4 h-4" />
              </Link>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl px-5 py-2">
              {reviews.length > 0
                ? reviews.slice(0, 3).map((r: any) => <ReviewCard key={r.id} review={r} />)
        ...
        <div className="grid xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold text-slate-900">Earnings This Month</h2>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-3xl font-extrabold text-slate-900">TK{(stats?.thisMonthEarnings ?? 0).toLocaleString()}</p>
              <p className="text-sm text-slate-400 mt-1">BDT earned this month</p>
            </div>
          </div>
          <EarningsChart />
        </div>
