...
import {
  ...
  Upload, Eye, Check, Bold, Italic, Underline,
...
type Section = { id: string; title: string; outcome: string; lectures: Lecture[]; collapsed: boolean; quizzes?: Quiz[] }
...
function EditCoursePage() {
  ...
  useEffect(() => {
    api.get(`/api/teacher/courses/${courseId}`)
      .then(({ data }) => {
        ...
        const hasParent    = !!c.category?.parent
        const categoryId   = hasParent ? (c.category.parent.id   ?? "") : (c.category?.id   ?? "")
        const categoryName = hasParent ? (c.category.parent.name ?? "") : (c.category?.name ?? "")
        const subId        = hasParent ? (c.category?.id   ?? "") : ""
        const subName      = hasParent ? (c.category?.name ?? "") : ""
        ...
        set({
          ...
          introVideo:     c.intro_video  ?? null,
          welcomeMessage: c.welcome_message  ?? "",
          congratsMessage:c.congrats_message ?? "",
          objectives: c.objectives?.filter((o: any) => o.type === "OBJECTIVE").map((o: any) => o.content).length
            ? c.objectives.filter((o: any) => o.type === "OBJECTIVE").map((o: any) => o.content)
            : [""],
          requirements: c.objectives?.filter((o: any) => o.type === "REQUIREMENT").map((o: any) => o.content).length
            ? c.objectives.filter((o: any) => o.type === "REQUIREMENT").map((o: any) => o.content)
            : [""],
          audience: c.objectives?.filter((o: any) => o.type === "TARGET_AUDIENCE").map((o: any) => o.content).length
            ? c.objectives.filter((o: any) => o.type === "TARGET_AUDIENCE").map((o: any) => o.content)
            : [""],
  ...
  async function updateCourse() {
    if (!w.title.trim())  { setSubmitError("Title required."); return }
    if (!w.categoryId)    { setSubmitError("Select category."); return }
    if (!w.subcategoryId) { setSubmitError("Select subcategory."); return }
    ...
    try {
      ...
      // 3. Sections + lessons: update existing (UUID id) or create new
      for (let si = 0; si < w.sections.length; si++) {
        ...
        if (isUuid(sec.id)) {
          await api.put(`/api/sections/${sec.id}`, { title: sec.title, order: si + 1 })
          sectionId = sec.id
        } else {
          const { data: secData } = await api.post(`/api/courses/${courseId}/sections`, { title: sec.title, order: si + 1 })
          sectionId = secData.data.section.id
        }
        ...
        for (let li = 0; li < sec.lectures.length; li++) {
          ...
          if (lec.duration)                            payload.duration   = parseInt(lec.duration) || 0
          if (lec.type === "video")                    payload.video_urls = lec.file_urls ?? []
          if (lec.type === "document")                 payload.file_urls  = lec.file_urls ?? []
          if (lec.type === "text" && lec.text_content) payload.content    = lec.text_content

          if (isUuid(lec.id)) {
            await api.put(`/api/lessons/${lec.id}`, payload)
          } else {
            await api.post(`/api/sections/${sectionId}/lessons`, payload)
          }
        }
      }
        ...
        <main className="flex-1">
          ...
          <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-extrabold text-slate-900 leading-tight">Edit Course</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={updateCourse} disabled={submitted || submitting}
                ...
                {submitted ? <><Check className="w-4 h-4" /> Saved!</> : submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving…</> : <><Check className="w-4 h-4" /> Save Changes</>}
              </button>
            </div>
          </div>
        ...
        {submitted && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
                ...
                <h3 className="text-lg font-extrabold text-slate-900 mb-2">Course updated successfully!</h3>
                <p className="text-sm text-slate-500">Your changes have been saved. Redirecting…</p>
              </div>
            </div>
        )}
      </div>
