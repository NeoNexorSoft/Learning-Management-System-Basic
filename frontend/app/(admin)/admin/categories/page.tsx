...
function CategoryForm({
                        initial, parents, onSubmit, loading, isParent,
                      }: {
...
function AdminCategoriesPage() {
  ...
  const parents = q
      ? allParents.filter((c) => c.name.toLowerCase().includes(q) || c.slug.toLowerCase().includes(q))
      : allParents
  ...
  const subs = q
      ? allSubs.filter((c) =>
          ...
          allParents.find((p) => p.id === c.parent_id)?.name.toLowerCase().includes(q)
      )
      : allSubs
  ...
  async function fetchCategories() {
    ...
    try {
      ...
      const msg = err?.response?.data?.message
          ?? (err?.code === "ERR_NETWORK" ? "Cannot reach the backend" : "Failed to load categories")
      setCategories([])
      showToast("error", msg)
    } finally {
      setLoading(false)
  ...
  function CategoryCard({ cat, isParent }: { cat: Category; isParent: boolean }) {
    const childCount = allSubs.filter((s) => s.parent_id === cat.id).length
    const parentName = isParent ? null : allParents.find((p) => p.id === cat.parent_id)?.name
    ...
    return (
        <div className="flex items-center justify-between p-4 border border-slate-200 rounded-xl hover:border-slate-300 hover:shadow-sm transition-all bg-white">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={() => openEdit(cat)} className="p-1.5 rounded-lg hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 transition-colors" title="Edit">
              <Pencil className="w-4 h-4" />
            </button>
            <button onClick={() => { setDeleteTarget(cat); setDeleteOpen(true) }} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-600 transition-colors" title="Delete">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {items.length} {q ? "results" : isParent ? "categories" : "subcategories"}
              </p>
            </div>
            <button onClick={onAdd} className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors">
            ...
            {loading ? (
                ...
                <div className="py-8 text-center">
                  <p className="text-sm text-slate-400">
                    {q ? `No results for "${search}"` : `No ${isParent ? "categories" : "subcategories"} yet.`}
                  </p>
                  {!q && (
                      <button onClick={fetchCategories} className="mt-2 text-xs font-semibold text-indigo-500 hover:text-indigo-700 underline transition-colors">
                        Retry loading
                      </button>
                  )}
                </div>
