        ...
        <div className="space-y-4">
            ...
            {shortQuestions.length > 0 && (
                <div className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
                    <span className="font-medium text-gray-700">{shortQuestions.length} টি সংক্ষিপ্ত প্রশ্ন</span> যোগ করা হয়েছে
                </div>
            )}
        ...
        <span className="text-xs font-bold text-orange-600 bg-orange-100 rounded px-1.5 py-0.5 shrink-0">
          {question.serial}
                ...
                {question.source === "database" && (
                    <span className="text-xs text-emerald-600 bg-emerald-100 rounded px-1.5 py-0.5 shrink-0">DB</span>
                )}
                <span className="text-xs text-gray-600 truncate flex-1">
          {question.text.slice(0, 60) || "নতুন সংক্ষিপ্ত প্রশ্ন"}
        </span>
                <div className="flex items-center gap-1 shrink-0">
                    <button type="button" onClick={() => setExpanded((v) => !v)} className="text-gray-400 hover:text-gray-600 p-0.5">
                        {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                    </button>
                    <button type="button" onClick={onRemove} className="text-gray-400 hover:text-red-500 p-0.5">
                        <Trash2 size={13} />
                    </button>
                </div>
            </div>
          ...
          />
                    <div className="flex items-center gap-2">
                        ...
                        <input
                            ...
                            onChange={(e) => onUpdate({ marks: parseInt(e.target.value) || 2 })}
                            min={1}
                            max={10}
